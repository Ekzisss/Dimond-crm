...
export const Text: FC<TextProps> = (props) => {
  const { size = 'md', variant = 'body', color = 'default', align, as, className, children, ...restProps } = props;
  ...
  const alignClass = align ? s[`align${align.charAt(0).toUpperCase() + align.slice(1)}`] : '';

  const Component = as || (variant === 'heading' ? 'h2' : 'p');

  return (
    <Component className={`${sizeClass} ${variantClass} ${colorClass} ${alignClass} ${className || ''}`} {...restProps}>
      {children}
    </Component>
  );
};
